...
export function GuildCard({
  ...
  return (
    <div className="card p-5 flex items-center justify-between gap-4 fx-hover-lift fx-fade-in">
      <div className="flex items-center gap-4 min-w-0">
        ...
        <div className="min-w-0">
          ...
          <div className="text-xs text-white/60">{botInGuild ? 'Bot ativo' : 'Bot nao esta no servidor'}</div>
        </div>
      </div>

      {botInGuild ? (
        <Link href={`/dashboard/${guild.id}`} className="btn-primary px-5 py-2 rounded-2xl fx-shimmer">
          Abrir
        </Link>
      ) : (
